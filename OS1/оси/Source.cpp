...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
	{
		...
		int x = GET_X_LPARAM(lParam) * N / rect.right; // номер квадрата, куда на который мы нажали, по x
		int y = GET_Y_LPARAM(lParam) * N / rect.bottom; // номер квадрата, куда на который мы нажали, по у
		if (matrix[x][y] == 0)
		{
			matrix[x][y] = 1; // если в нет нет крестика, то скажем, что надо поставить круг
	...
	case WM_RBUTTONDOWN:
	{
		...
		if (matrix[x][y] == 0)
		{
			matrix[x][y] = -1; // если в нет нет круга, то скажем, что надо поставить крестик
			InvalidateRect(hwnd, NULL, TRUE); // вызываем WM_PAINT
		}
