...
WM_CHECKENDGAME; // вызывается после каждого хода

bool BGP = true; // BackGround Paint
int TC = 0; // Turn Count
bool xWin = false, oWin = false;
HANDLE Painting; // дескриптор потока
...
bool WinCheck()
{
	...
	for (int i = 0; i < N; i++) // строки
	{
		for (int j = 0; j < N; j++)
		{
			if (matrix[i][j] == -1) oCount++;
			if (matrix[i][j] == 1) xCount++;
		}
		if (oCount == N)
		{
			oWin = true;
	...
	{
		if (matrix[i][i] == -1) oCount++;
		if (matrix[i][i] == 1) xCount++;
	}
	if (oCount == N)
	{
		oWin = true;
	...
	for (int i = N - 1; i >= 0; i--) // побочная диагональ
	{
		if (matrix[i][i] == -1) oCount++;
		if (matrix[i][i] == 1) xCount++;
	}
...
DWORD WINAPI BackGroundPaint(LPVOID lpParam) // ThreadProc callback function
{
	while (true)
	{
		if (BGP)
...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	if (Msg == WM_CHECKENDGAME) // проверка на конец игры и победителя
	{
		if (WinCheck()) // если кто-то победил
		{
			if (oWin)
			{
				MessageBox(hwnd, L"CIRCLES WIN!", L"TicTacToe", MB_OK | MB_SETFOREGROUND | MB_TASKMODAL);
				SendMessage(hwnd, WM_DESTROY, NULL, NULL);
			}
			if (xWin)
			{
				MessageBox(hwnd, L"CROSSES WIN!", L"TicTacToe", MB_OK | MB_SETFOREGROUND | MB_TASKMODAL);
				SendMessage(hwnd, WM_DESTROY, NULL, NULL);
			}
		}
		else // если никто не победил
		{
			if (FullMatrix()) // но все поля заполнены
			{
				
				MessageBox(hwnd, L"NO WINNER!", L"TicTacToe", MB_OK | MB_SETFOREGROUND | MB_TASKMODAL);
				SendMessage(hwnd, WM_DESTROY, NULL, NULL);
	...
	if (Msg == WM_CROSS) // ставим крест в поле
	{
		...
		if (matrix[i][j] == 0)
		{
			...
			TC++;
			SendMessage(hwnd, WM_CHECKENDGAME, wParam, lParam);
		}
	...
	case WM_LBUTTONDOWN:
	{
		...
		if (TC % 2 == 1) // если ход ноликов, то рассказываем всем процессам о том, что мы нажали лкм
		{
			SendMessage(HWND_BROADCAST, WM_CIRCLE, wParam, MAKELPARAM(x, y)); // отпралвяем сообщение о нажатии лкм
		}
		else // иначе говорим, что сейчас ход крестиков
		{
			MessageBox(hwnd, L"It's crosses' turn!", L"TicTacToe", MB_OK);
		}
	...
	case WM_RBUTTONDOWN:
	{
		...
		int x = GET_X_LPARAM(lParam) * N / rect.right; // номер квадрата, на который мы нажали, по x
		int y = GET_Y_LPARAM(lParam) * N / rect.bottom; // номер квадрата, на который мы нажали, по у
		if (TC % 2 == 0) // если ход крестиков, то рассказываем всем процессам о том, что мы нажали пкм
		{
			SendMessage(HWND_BROADCAST, WM_CROSS, wParam, MAKELPARAM(x, y)); // отпралвяем сообщение о нажатии пкм
		}
		else // иначе говорим, что сейчас ход ноликов
		{
			MessageBox(hwnd, L"It's circles' turn!", L"TicTacToe", MB_OK);
		}
	...
	case WM_KEYDOWN: // сообщение о нажатой клавише на клаве
	{
		...
		{
			BGP = !BGP;
			if (BGP)  MessageBox(hwnd, L"Rendering thread is resume.", L"TicTacToe", MB_OK);
			else MessageBox(hwnd, L"Rendering thread is suspended.", L"TicTacToe", MB_OK);
			break;
		}
		case '1':
			SetThreadPriority(Painting, THREAD_PRIORITY_IDLE);
			MessageBox(hwnd, L"Rendering thread priority is idle.", L"TicTacToe", MB_OK);
			break;
		case '2':
			SetThreadPriority(Painting, THREAD_PRIORITY_LOWEST);
			MessageBox(hwnd, L"Rendering thread priority is lowest.", L"TicTacToe", MB_OK);
			break;
		case '3':
			SetThreadPriority(Painting, THREAD_PRIORITY_BELOW_NORMAL);
			MessageBox(hwnd, L"Rendering thread priority is below normal.", L"TicTacToe", MB_OK);
			break;
		case '4':
			SetThreadPriority(Painting, THREAD_PRIORITY_NORMAL);
			MessageBox(hwnd, L"Rendering thread priority is normal.", L"TicTacToe", MB_OK);
			break;
		case '5':
			SetThreadPriority(Painting, THREAD_PRIORITY_ABOVE_NORMAL);
			MessageBox(hwnd, L"Rendering thread priority is above normal.", L"TicTacToe", MB_OK);
			break;
		default:
			break;
		}
